...
剪贴板捕获默认关闭。你可以在设置中开启“快捷键打开时读取剪贴板”，之后使用配置好的全局快捷键打开快速记录窗口。如果当前草稿为空，并且剪贴板中有文字，IdeaCard 会把文字填入正文。

这个开关会在主窗口和快速记录窗口之间即时同步，修改后不需要重启应用，也不需要等待窗口重新聚焦。
